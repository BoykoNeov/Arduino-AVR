...
// this will be hashed, max size in bytes
uint8_t input[64];
uint32_t hashCount = 0;
char charPrint[65];
uint8_t paddedInput[64]= {};

void PadBlock(byte blockLength, uint32_t messageLength)
{
    //copy input
  ...
  for (uint8_t i = blockLength; i < 64; i++)
  {
    paddedInput[i] = 0;
  }

  uint64_t inputLengthInBits = messageLength * 8;
...
void loop()
{
    ...
    hashCount++;
    PadBlock(stringLength, stringLength);
		CalculateSHA256Block();
    CopyHashToPrintBuffer();
    PrintHash();
    CopyHashToInput();
