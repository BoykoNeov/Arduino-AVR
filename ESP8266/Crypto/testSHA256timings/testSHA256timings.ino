// ESP8266 is about 80 times faster than atmega328p using this implementation

uint32_t const roundConstants[] = {
  ...
  uint32_t const originalOverallHashes[] = {
    0x6A09E667,
    0xBB67AE85,
    0x3C6EF372,
    0xA54FF53A,
...
{
  Serial.begin(9600);
  Serial.println(String("Mhz: ") + ESP.getCpuFreqMHz());
}
...
void loop()
{
  ...
  if (userInput != NULL)
  {
    ...
    while(true)
    {
      ...
      {
        Serial.print(String("secs per 10000 hashes: ") + (float)(millis() - oldMillis) / 1000 + (String(" ")));
        oldMillis = millis();
        PrintHash();
        // required for the software watchdog timer, otherwise it resets the processor
        yield();
      }
