//now when i think about this, this is hard to test the way it is implemented, and not very practical, but can serve for future implementation
//this is limited to 64 byte input
...
void loop()
{
    ...
    {
      input[i / 2] = HexChars2byte(userInput[i], userInput[i + i]);
    }
...
char* serialString()
{
	static char str[64]; // For strings of max length=64
...
uint8_t HexChars2byte(char firstChar, char secondChar)
{
	char chars[2];
	chars[0] = firstChar;
	chars[1] = secondChar;
	...
	uint8_t result = output[0] * 16 + output[1];
	return result;
}
